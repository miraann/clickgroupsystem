    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-sm bg-[#0d1220] border border-white/15 rounded-2xl shadow-2xl flex flex-col max-h-[80vh]">
        <div className="flex items-center justify-between px-5 pt-5 pb-3 shrink-0">
          <h3 className="text-base font-bold text-white">{t.pay_select_member}</h3>
        ...
        <div className="px-5 pb-3 shrink-0">
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)}
              placeholder={t.pay_search_name_phone} autoFocus
          ...
          {filtered.map(m => (
            <button key={m.id} onClick={() => { onSelect(m); onClose() }}
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-amber-400/60 mt-0.5">{m.points} {t.pay_pts}</p>
              </div>
              {selectedId === m.id && <Check className="w-4 h-4 text-amber-400 shrink-0" />}
            </button>
          ))}
          {list.length === 0 && (
            <p className="text-center text-white/30 text-sm py-8">{t.pay_no_members}</p>
          )}
        </div>
      </div>
    </div>
