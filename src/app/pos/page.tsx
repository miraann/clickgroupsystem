...
export default function POSEntry() {
  ...
  const [restaurantName, setRestaurantName] = useState('Point of Sale')
  ...
  // Load staff in the background — no blocking spinner
  useEffect(() => {
    const load = async () => {
      try {
    ...
    <div className="min-h-screen bg-[#060810] flex flex-col items-center justify-center overflow-hidden relative select-none">
      ...
      <div className="fixed top-0 left-0 right-0 z-10 flex items-center justify-between px-8 py-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-white leading-none">{restaurantName}</p>
        ...
        <div className="relative z-10 w-full max-w-2xl px-4">
          <div className="text-center mb-10">
            ...
            <p className="text-white/35 mt-2">
              {staff.length === 0 ? 'Loading staff...' : 'Tap your name to sign in'}
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            {staff.map((s) => (
              <button
                ...
                className="group relative rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-5 hover:bg-white/10 hover:border-white/20 transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] text-left touch-manipulation"
              >
        ...
        <div className="relative z-10 w-full max-w-sm px-4">
          <div className="text-center mb-8">
            <button
              ...
              className="inline-flex flex-col items-center gap-3 group touch-manipulation"
            >
              <div className={cn(
              ...
              <div>
                ...
                <p className="text-sm text-white/40 capitalize">{selectedStaff.role} · tap to switch</p>
              </div>
            </button>
          </div>
            ...
            {[0, 1, 2, 3].map(i => (
              <div key={i} className={cn(
                ...
                success ? 'bg-emerald-400 border-emerald-400 scale-110'
                  : error ? 'bg-rose-400 border-rose-400'
            ...
            <div className="grid grid-cols-3 gap-3">
              {KEYS.map((key) => (
                <button
                  ...
                  disabled={success}
                  className={cn(
                    'relative h-16 rounded-2xl font-semibold text-xl transition-all duration-100 touch-manipulation active:scale-95 disabled:opacity-50',
                    key === 'del' || key === 'clear'
