...
const RETENTION_OPTIONS = [
  { value: 30,  labelKey: 'db_ret_30'  as const },
  { value: 60,  labelKey: 'db_ret_60'  as const },
  { value: 90,  labelKey: 'db_ret_90'  as const },
  { value: 180, labelKey: 'db_ret_6mo' as const },
...
export default function DatabasePage() {
  ...
  const [stats, setStats] = useState<{ labelKey: StatKey; count: number; icon: React.ReactNode; color: string }[]>([])
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setStats([
        { labelKey: 'db_orders',       count: orders.count       ?? 0, icon: <ShoppingCart   className="w-4 h-4" />, color: 'text-white bg-amber-500/70   border-amber-500/40'   },
        { labelKey: 'db_customers',    count: customers.count    ?? 0, icon: <Users           className="w-4 h-4" />, color: 'text-white bg-indigo-500/70  border-indigo-500/40'  },
        { labelKey: 'db_menu_items',   count: menu.count         ?? 0, icon: <UtensilsCrossed className="w-4 h-4" />, color: 'text-white bg-emerald-500/70 border-emerald-500/40' },
        { labelKey: 'db_staff',        count: staff.count        ?? 0, icon: <UserCheck       className="w-4 h-4" />, color: 'text-white bg-violet-500/70  border-violet-500/40'  },
        { labelKey: 'db_members',      count: members.count      ?? 0, icon: <Package         className="w-4 h-4" />, color: 'text-white bg-cyan-500/70    border-cyan-500/40'    },
        { labelKey: 'db_reservations', count: reservations.count ?? 0, icon: <CalendarDays    className="w-4 h-4" />, color: 'text-white bg-rose-500/70    border-rose-500/40'    },
      ])
  ...
  return (
    <motion.div variants={CONTAINER} initial="hidden" animate="show" className="max-w-2xl space-y-6 pb-10">
      ...
      <motion.div variants={ITEM} className="flex items-start gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">{t.db_title}</h1>
          <p className="text-sm text-white/40 mt-1">{t.db_subtitle}</p>
        </div>
      </motion.div>
      ...
      <motion.div variants={ITEM} className="space-y-3">
        <p className="text-xs font-semibold text-white/30 uppercase tracking-widest flex items-center gap-2">
          <HardDrive className="w-3.5 h-3.5" /> {t.db_storage_overview}
        </p>
        <div className="grid grid-cols-3 gap-3">
          {statsLoading
            ...
            : stats.map(s => (
                <div key={s.labelKey} className={cn('rounded-xl border px-4 py-3 flex items-center gap-3', s.color)}>
                  {s.icon}
                  <div>
      ...
      <motion.div variants={ITEM} className="rounded-2xl border border-white/10 bg-white/3 backdrop-blur-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-white/8 flex items-center gap-3">
          ...
          <p className="text-sm font-semibold text-white">{t.db_export_backup}</p>
        </div>
        <div className="px-5 py-4 flex items-center justify-between gap-4">
          <div>
            <p className="text-sm text-white/70">{t.db_download_full_backup}</p>
            <p className="text-xs text-white/35 mt-0.5">
              {t.db_download_full_backup_desc}
            </p>
          </div>
          <button
          ...
          >
            ...
            {exporting ? t.db_exporting : t.db_download}
          </button>
        </div>
      </motion.div>
      ...
      <motion.div variants={ITEM} className="rounded-2xl border border-white/10 bg-white/3 backdrop-blur-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-white/8 flex items-center gap-3">
          ...
          <p className="text-sm font-semibold text-white">{t.db_data_retention}</p>
        </div>
        <div className="px-5 py-4 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-white/70">{t.db_auto_cleanup}</p>
              <p className="text-xs text-white/35 mt-0.5">
                {t.db_auto_cleanup_desc}
              </p>
            </div>
            <ToggleSwitch
          ...
          <AnimatePresence>
            {settings.retention_enabled && (
              ...
              >
                <div>
                  <p className="text-xs text-white/40 font-medium mb-2">{t.db_keep_records_for}</p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  >
                    ...
                    {cleaning ? t.db_running : t.db_run_cleanup}
                  </button>
      ...
      <motion.div variants={ITEM} className="rounded-2xl border border-rose-500/20 bg-rose-500/5 backdrop-blur-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-rose-500/15 flex items-center gap-3">
          ...
          <p className="text-sm font-semibold text-white">{t.db_gdpr_title}</p>
        </div>
        <div className="px-5 py-4 space-y-3">
          <p className="text-sm text-white/60 leading-relaxed">
            {t.db_gdpr_desc_1}{' '}
            <span className="text-rose-400 font-medium">{t.db_gdpr_cannot_undo}</span>
          </p>
          ...
          <AnimatePresence>
            {gdprDone && (
              ...
              >
                <Check className="w-3.5 h-3.5" /> {t.db_gdpr_deleted_ok}
              </motion.p>
            )}
          </AnimatePresence>

          <button
            onClick={() => { setGdprModal(true); setGdprConfirm(''); setGdprError(false) }}
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          {restoreState === 'parsing' && (
            <div className="py-8 flex items-center justify-center gap-2 text-white/40 text-sm">
              <Loader2 className="w-4 h-4 animate-spin" /> {t.db_parsing}
            </div>
          )}

          {restoreState === 'preview' && (
            <div className="space-y-3">
              <p className="text-xs font-semibold text-white/35 uppercase tracking-widest">{t.db_found_in_backup}</p>
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(restorePreview).map(([table, count]) => (
                  <div key={table} className="flex items-center justify-between px-3 py-2 rounded-lg bg-white/5 border border-white/8">
                    ...
                    <span className="text-xs font-medium text-white/80">{count.toLocaleString()} {t.db_rows}</span>
                  </div>
                ))}
              </div>
          ...
          {restoreState === 'restoring' && (
            <div className="py-8 flex items-center justify-center gap-2 text-violet-400 text-sm">
              <Loader2 className="w-4 h-4 animate-spin" /> {t.db_restoring}
            </div>
          )}
          ...
          >
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-bold text-white">{t.db_modal_title}</h2>
                  <p className="text-xs text-white/40">{t.db_modal_subtitle}</p>
                </div>
              </div>

              <p className="text-sm text-white/55 leading-relaxed">
                {t.db_modal_will_delete}{' '}
                <span className="text-white/80">{t.db_modal_tables}</span>.
              </p>

              <div>
                <label className="text-xs text-white/40 block mb-1.5">
                  {t.db_modal_enter_pin}
                </label>
                <input
                  type="password"
                  inputMode="numeric"
              ...
              <div className="flex gap-3">
                ...
                >
                  ...
                  {gdprDeleting ? t.db_deleting : t.db_delete_all}
                </button>
              </div>
            </motion.div>
          </motion.div>
