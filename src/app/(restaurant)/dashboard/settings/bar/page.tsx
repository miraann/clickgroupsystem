  ...
  return (
    <motion.div variants={PAGE} initial="hidden" animate="show" className="space-y-6 max-w-2xl">
      ...
      <div className="flex items-center justify-between">
        ...
        <FadeSwitch id={loading ? 'skel' : 'btn'}>
          {loading ? <Skel className="h-9 w-24 rounded-xl" /> : <SaveButton state={saveState} onClick={save} />}
        </FadeSwitch>
      </div>

      {/* ── General ── */}
      <motion.div variants={FIELD_ITEM}>
        <FadeSwitch id={loading ? 'skel-general' : 'general'}>
          {loading ? <SkelSection lines={2} /> : (
      ...
      {/* ── Happy Hour ── */}
      <motion.div variants={FIELD_ITEM}>
        <FadeSwitch id={loading ? 'skel-happy' : 'happy'}>
          {loading ? <SkelSection lines={3} /> : (
      ...
      {/* ── Tab Auto-close ── */}
      <motion.div variants={FIELD_ITEM}>
        <FadeSwitch id={loading ? 'skel-tab' : 'tab'}>
          {loading ? <SkelSection lines={2} /> : (
      ...
      {/* ── Bar Note ── */}
      <motion.div variants={FIELD_ITEM}>
        <FadeSwitch id={loading ? 'skel-note' : 'note'}>
          {loading ? <SkelSection lines={2} /> : (
