...
export default function DiscountPage() {
  ...
  const handleSave = async () => {
    ...
    if (editId) {
      ...
      if (!error) setDiscounts(ds => ds.map(d => d.id === editId ? { ...d, ...payload } : d))
    } else {
      ...
      const { data, error } = await supabase
        ...
        .select().single()
      if (!error && data) setDiscounts(ds => [...ds, data as Discount])
    }
    setSaving(false)
    setModal(false)
  }
  ...
  const toggleActive = async (d: Discount) => {
    ...
    setDiscounts(ds => ds.map(x => x.id === d.id ? { ...x, active: newVal } : x))
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!error) setDiscounts(ds => ds.filter(d => d.id !== id))
    setDeleteId(null)
  }
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-white">Discounts</h1>
            <p className="text-xs text-white/40">Percentage and fixed discounts</p>
          </div>
        ...
        {discounts.map(d => (
          <div key={d.id} className={cn('flex items-center gap-3 p-4 bg-white/5 border rounded-2xl transition-all', d.active ? 'border-white/10' : 'border-white/5 opacity-60')}>
            ...
            <button onClick={() => handleDelete(d.id)} className={cn('h-8 rounded-lg flex items-center justify-center transition-all active:scale-95 text-xs font-medium shrink-0',
              ...
              {deleteId === d.id ? 'Confirm?' : <Trash2 className="w-3.5 h-3.5" />}
            </button>
          </div>
        ))}
        {discounts.length === 0 && <div className="text-center py-16 text-white/25 text-sm">No discounts yet.</div>}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md bg-[#0d1220]/95 backdrop-blur-2xl border border-white/15 rounded-3xl p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-semibold text-white">{editId ? 'Edit Discount' : 'Add Discount'}</h2>
