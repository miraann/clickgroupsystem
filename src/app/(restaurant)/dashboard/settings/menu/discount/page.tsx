...
export default function DiscountPage() {
  ...
  return (
    <motion.div key="menu-discount-page" variants={PAGE} initial="hidden" animate="show" exit="exit" className="max-w-5xl mx-auto">
      {/* Header */}
      ...
      <AnimatedList className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2.5">
        {/* Add-new card */}
        <button onClick={openAdd}
          ...
          <span className="text-[11px] font-semibold">{t.disc_add}</span>
        </button>
        {discounts.map(d => {
          const armed = deleteId === d.id
          return (
            <AnimatedItem key={d.id} className={cn('flex flex-col items-center rounded-xl border bg-white/5 px-2.5 py-2.5 text-center transition-colors',
              d.active ? 'border-white/10 hover:border-white/20' : 'border-white/5 opacity-55')}>
              ...
              <p className="mt-1.5 w-full text-sm font-bold text-white line-clamp-1">{d.name}</p>
              <p className="text-[10px] text-white/35 line-clamp-2 w-full">
                {d.type === 'percentage' ? `${d.value}% off` : `$${Number(d.value).toFixed(2)} off`}
                {d.min_order > 0 && ` · Min $${Number(d.min_order).toFixed(0)}`}
              </p>
              ...
              <div className="flex items-start justify-center gap-2 card-actions">
                ...
                <div className="flex flex-col items-center gap-1">
                  <button onClick={() => toggleActive(d)}
                    className={cn('w-8 h-8 rounded-lg flex items-center justify-center transition-all active:scale-95',
                      d.active ? 'bg-emerald-500/15 text-emerald-400 hover:bg-emerald-500/25' : 'bg-white/5 text-white/30 hover:bg-white/10')}>
        ...
        {discounts.length === 0 && <div className="col-span-full text-center py-12 text-white/25 text-sm">{t.disc_no_data}</div>}
      </AnimatedList>
