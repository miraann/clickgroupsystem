...
import type { DbOrderItem, DbMenuItem, DraftEntry, KitchenNote } from '../types'
...
export function OrderPanel({
  ...
  draft, draftEntries, draftTotal, kitchenNotes,
  ...
  return (
    <div className={cn(
      ...
      <div className="shrink-0 flex border-b border-white/8">
        {(['ordering', 'ordered'] as const).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className={cn('flex-1 py-3.5 sm:py-4 text-xs sm:text-sm font-bold uppercase tracking-wider transition-all relative touch-manipulation',
      ...
      <div className="flex-1 overflow-y-auto">
        {activeTab === 'ordering' ? (
          draftEntries.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-3 sm:gap-4 pb-10">
              <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-white/4 border border-white/8 flex items-center justify-center">
                <ShoppingBag className="w-7 h-7 sm:w-9 sm:h-9 text-white/15" />
              </div>
              <p className="text-sm sm:text-base text-white/25">{tr.ord_no_items_yet}</p>
              <p className="text-xs sm:text-sm text-white/15">{tr.ord_tap_menu}</p>
            </div>
          ) : (
          ...
          sentItems.length === 0 ? (
            loading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full gap-3 sm:gap-4 pb-10">
                <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-white/4 border border-white/8 flex items-center justify-center">
                  <ChefHat className="w-7 h-7 sm:w-9 sm:h-9 text-white/15" />
                </div>
                <p className="text-sm sm:text-base text-white/25">{tr.ord_nothing_sent}</p>
              </div>
            )
          ) : (
            <div className="p-3 sm:p-4 space-y-2 sm:space-y-2.5">
      ...
      {activeTab === 'ordering' && draftEntries.length > 0 && (
        <div className="shrink-0 px-4 py-3 sm:px-5 sm:py-4 border-t border-white/8 flex items-center justify-between">
          <span className="text-xs sm:text-sm text-white/30">{draftEntries.reduce((s, o) => s + o.entry.qty, 0)} {tr.ord_items}</span>
          <span className="text-sm sm:text-base font-bold text-white/60 tabular-nums">{formatPrice(draftTotal)}</span>
        </div>
      )}
      {activeTab === 'ordered' && sentItems.length > 0 && (
        <div className="shrink-0 px-4 py-3 sm:px-5 sm:py-4 border-t border-white/8 flex items-center justify-between">
          ...
          <span className="text-sm sm:text-base font-bold text-white/60 tabular-nums">{formatPrice(sentTotal)}</span>
        </div>
      )}
    </div>
  )
