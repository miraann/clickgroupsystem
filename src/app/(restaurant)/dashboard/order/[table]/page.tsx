...
// ── Kitchen print popup ────────────────────────────────────────
// Opens a small dark-themed popup with the kitchen ticket.
// Uses window.print() triggered by a user-click button (Chrome blocks
// auto-print from popups; a button click is a trusted gesture).
async function printKitchenTicket(params: {
  restaurantId: string
  tableNum:     string
...
<div id="receipt">
  ...
  // Position popup at top-right so it doesn't cover the order screen
  const left = Math.max(0, (screen.width  ?? 1280) - 360)
  const top  = 60
  const win  = window.open(url, '_blank', `width=330,height=460,left=${left},top=${top}`)
...
function OrderPage() {
  ...
  const handleSend = async () => {
    ...
    if (error) {
      ...
      // Kitchen ticket print — fire and forget (DB insert already succeeded)
      if (restaurantId) {
        printKitchenTicket({
          restaurantId,
          tableNum: table,
